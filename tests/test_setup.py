...
from local_app.benchling_app.setup import (
    _auth_method,
    app_definition_id,
    init_app_from_webhook,
)
...
class TestBenchlingAppSetup:
    ...
    def test_init_app_from_webhook(self, monkeypatch) -> None:
        ...
        with monkeypatch.context() as context:
            ...
            context.setenv(
                "CLIENT_SECRET_FILE", str(_TEST_FILES_PATH / "test_client_secret")
            )
    ...
    def test_init_app_from_webhook_missing_client_id(self, monkeypatch) -> None:
        ...
        with monkeypatch.context() as context:
            ...
            with pytest.raises(
                AssertionError, match="Missing CLIENT_ID from environment"
            ):
                init_app_from_webhook(webhook)

    def test_init_app_from_webhook_missing_client_secret_file(
        self, monkeypatch
    ) -> None:
        webhook = load_webhook_json(_TEST_FILES_PATH / "canvas_initialize_webhook.json")
        with monkeypatch.context() as context:
            context.setenv("CLIENT_ID", "clientId")
            with pytest.raises(
                AssertionError, match="Missing CLIENT_SECRET_FILE from environment"
            ):
                init_app_from_webhook(webhook)
    ...
    def test_app_definition_id_missing_app_definition_id(self, monkeypatch) -> None:
        with (
            monkeypatch.context(),
            pytest.raises(
                AssertionError, match="Missing APP_DEFINITION_ID from environment"
            ),
        ):
            app_definition_id()
